...
# Updates from original microclimdata

## Climate

era5_download: Downloads climate data from era5 land except for surface direct solar radiation (fdir), mean surface downward long-wave radiation flux (avg_sdlwrf), and land-sea mask (lsm). These variables are not currently available from era5 land and are therefore downloaded from era5 reanalysis at a coarser resolution. Now returns a list of length 2, with the list of era5 reanalysis requests and the list of era5 land requests.

era5_process: Adjusted to accommodate the combination of era5 reanalysis and era5 land. nc should not be a vector with two file paths: the first is the path to the reanalysis nc file and the second is the path to the era5 land file. Within .extract_clima, variables from era5 reanalysis are resampled to resolution and extent of era5 land before processing.

To check: in .extract_clima the coefficient for correction of diurnal temperature ranges near water is 1.285. Check if this is still accurate for ERA5 land.

## Vegetation

GEDI data provides information about the vertical distribution of plant area index. There are two options for downloading GEDI data. 1) From orbits: `gedi_download()` will download orbits with and extent that falls within the specified area of interest. Downloading filters to only points with l2b_quality_flag = 0 and power beams. 2) Alternatively, you can download gedi data in tiles using `gedi_download_tiles()`, but the temporal extent is limited to 2019 - 2022. These points are from Burns et al. 2024, and are already filtered to high quality points, but include power and coverage beams. After downloading the data, run `gedi_process()`. If you downloaded points from orbits set type = 'points', while if you downloaded points from tiles set type = 'tiles'. If type = 'tiles' beams will automatically be filtered to only include power beams. Use the list of filters as described in the function documentation to filter the data as you wish.
